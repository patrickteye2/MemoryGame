...
class Game {
	...
	checkWin() {
		if (this.board.filter((card) => card.found === false).length === 0) {
			...
			if (this.players == 1) {
				...
				popup.querySelector('.popup-head').innerHTML = `
	...
	handleClick(e) {
		if (e.target.classList.contains('card')) {
			...
			this.theme === 'numbers' ? (content = target.value) : (content = `<i class="fa-solid ${icons[target.value - 1]}"></i>`)
...
function restart() {
	if (gameState !== 'setup') {
		...
		document.querySelector('.menu-mobile-nav').classList.remove('show')
		clearGame()
		startGame()
	}
}
...
function handleMenuMobile() {
	document.querySelector('.menu-mobile-nav').classList.toggle('show')
}
